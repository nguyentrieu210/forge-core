...
const sourcePrint = brief.prints.find((entry) => entry.doctype === "Delivery Note" && entry.default);
assert.ok(sourcePrint, "thiếu mẫu in Delivery Note mặc định");
...
const fixture = {
  ...
  data: {
    ...
    driver: "Lê Văn Nam",
    vehicle: "51D-123.45",
    note: "Giao buổi sáng. Khách yêu cầu kiểm tra remote, hành trình đóng mở và bàn giao hướng dẫn sử dụng trước khi ký.",
    items: [
      {
        idx: 2,
        item_code: "REMOTE-01",
...
test("Alumdoor Delivery Note print keeps the A4 handover contract", () => {
  const css = print.css;
  ...
  assert.equal(print.name, "Phiếu giao hàng / lắp đặt ALUMDOOR");
...
test("Alumdoor Delivery Note renders long delivery data through the real renderer", () => {
