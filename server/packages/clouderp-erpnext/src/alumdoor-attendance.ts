 ...
 * that understands Ca 1, Ca 2 and automatic Ca 3 overtime.  It is written solely by
 * the authenticated attendance scan transaction; ordinary resource saves cannot
 * manufacture payable minutes.
 */
...
export class AlumDoorAttendanceDayController implements DocumentController<JsonObject> {
  ...
  async buildPlan(context: ControllerContext<JsonObject>): Promise<MutationPlan<JsonObject>> {
    if (!context.command.actor.roles.includes(INTERNAL_SCAN_ROLE)) {
      throw errors.permission("AlumDoor Attendance Day chỉ được cập nhật từ giao dịch quét QR đã xác thực.");
    }
    if (context.command.action !== "create" && context.command.action !== "save") {
      throw errors.lifecycle("AlumDoor Attendance Day chỉ hỗ trợ tạo hoặc cập nhật từ giao dịch quét QR.");
    }

    const data = await normalizeAttendanceDay(context);
    ...
    return {
      ...
      fulfillment_entries: [],
      events: [domainEvent({
        type: `alumdoor_attendance_day.${context.command.action === "create" ? "created" : "updated"}`,
...
async function normalizeAttendanceDay(context: ControllerContext<JsonObject>): Promise<JsonObject> {
  ...
  try {
    calculated = calculateAttendance({
      ...
      timeZone: timezone,
      windows,
      regularDailyCapMinutes,
    });
  } catch (error) {
    if (error instanceof AttendanceRuleError) {
      throw errors.validation(error.message, { code: error.code });
    }
    throw error;
  }
  ...
  const existingByCode = new Map(
    Array.isArray(input.segments)
      ? input.segments
        ...
        .map((value) => [String(value.segment_code ?? ""), value])
      : [],
  );
...
function segmentSnapshots(value: unknown): SegmentInput[] {
  ...
  for (const raw of value) {
    ...
    segments.push({
      ...
      ...(actualIn ? { actualIn } : {}),
      ...(actualOut ? { actualOut } : {}),
      ...(typeof row.row_id === "string" ? { row_id: row.row_id } : {}),
      ...(typeof row.in_checkin === "string" ? { in_checkin: row.in_checkin } : {}),
      ...(typeof row.out_checkin === "string" ? { out_checkin: row.out_checkin } : {}),
    });
  }
...
function optionalDatetime(value: unknown, field: string): string | undefined {
  ...
  return text ? H.requiredDatetime(text, field) : undefined;
}

function policyWindows(policy: JsonObject): AttendanceSegmentWindow[] {
  const shift1Start = H.integer(policy.shift1_start_minute, 420);
  const shift1End = H.integer(policy.shift1_end_minute, 690);
  const shift2Start = H.integer(policy.shift2_start_minute, 780);
  const shift2End = H.integer(policy.shift2_end_minute, 1020);
  const shift3Start = H.integer(policy.shift3_start_minute, 1050);
  const shift3End = H.integer(policy.shift3_latest_out_minute, 1439) + 1;
  return [
...
function childRows(data: JsonObject): ChildRow[] {
  ...
  return segments.map((segment, index) => {
    ...
    return {
      fieldname: "segments",
      child_doctype: SEGMENT_DOCTYPE,
      row_id: String(row.row_id ?? `SEGMENT-${index + 1}`),
      idx: index + 1,
