...
export interface AlumDoorPayrollApprovalInput {
  tenantId: string;
  actor: Actor;
  payrollEntry: string;
}

export interface AlumDoorPayrollCoordinatorServices {
...
/**
 * Final payroll approval is one transaction. Salary slips are recalculated and
 * submitted first, source Attendance Days are then locked to this period, and the
 * canonical Payroll Entry is submitted last. DocumentKernel.executeBundle provides
 * the same staged-state semantics used by the QR checkin create+submit bundle.
 */
export async function approveAlumDoorPayroll(
  input: AlumDoorPayrollApprovalInput,
  services: AlumDoorPayrollCoordinatorServices,
): Promise<JsonObject> {
  ...
  if (!["calculated", "pending_approval"].includes(text(payroll.data.alu_state))) {
    throw errors.lifecycle("Payroll Entry must be calculated or pending approval before approval");
  }
  ...
  const systemActor: Actor = {
    ...input.actor,
    roles: [...new Set([...input.actor.roles, INTERNAL_PAYROLL_ROLE, "Payroll Manager", "HR Manager", "Accounts Manager"])],
  };
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (text(slip.data.company) !== company || text(slip.data.start_date) !== startDate || text(slip.data.end_date) !== endDate) {
      throw errors.reference(`Salary Slip ${slipName} does not belong to Payroll Entry ${payrollName}`);
    }
    if (text(slip.data.alu_payroll_entry) !== payrollName) {
      throw errors.reference(`Salary Slip ${slipName} is not bound to Payroll Entry ${payrollName}`);
    }
    ...
    if (slip.docstatus === 0) {
      commands.push(await command({ tenantId: input.tenantId, actor: systemActor, doctype: SALARY_SLIP, name: slipName, action: "submit", expectedVersion: slip.version, document: { ...slip.data, alu_state: "pending_approval" }, submittedAt: now, commandId: `alu-payroll:${payrollName}:slip:${slipName}:submit` }));
    }
  }

  const attendance = (await services.store.listDocumentsByDoctype<JsonObject>(input.tenantId, ATTENDANCE_DAY))
    .filter((entry) => employees.has(text(entry.data.employee))
      && text(entry.data.work_date) >= startDate
      && text(entry.data.work_date) <= endDate)
  ...
  for (const day of attendance) {
    const state = text(day.data.state);
    const lockedBy = text(day.data.locked_by_payroll);
    if (state === "locked" && lockedBy === payrollName) continue;
    if (!["complete", "approved"].includes(state)) {
      throw errors.validation(`PAYROLL_BLOCKED: Attendance Day ${day.name} is ${state || "invalid"}`);
    }
  ...
  commands.push(await command({
    ...
    document: { ...payroll.data, alu_state: "pending_approval", alu_input_hash: aggregateHash, alu_approved_by: input.actor.user_id, alu_approved_at: now },
    submittedAt: now,
    commandId: `alu-payroll:${payrollName}:approve`,
  }));
  ...
  return {
    replayed: false,
    payroll_entry: payrollName,
    state: "approved",
    employee_count: employees.size,
    attendance_locked: attendance.length,
    input_hash: aggregateHash,
    version: payrollReceipt?.version ?? payroll.version + 1,
  };
}

function assertApprover(actor: Actor): void {
  const allowed = new Set(["AlumDoor Payroll Approver", "HR Manager", "System Manager", "Administrator"]);
  if (!actor.roles.some((role) => allowed.has(role)) && actor.user_id !== "Administrator") {
    throw errors.permission("AlumDoor Payroll Approver is required to approve payroll");
  }
}
