...
| policy_name | TEXT | NOT NULL | `z.string().trim().min(3).max(120)` | text `*` | `requiredTrimmed`: "Nhập tên chính sách" | `Ca AlumDoor 8 giờ` ở setup lần đầu | all thấy; Manager+ sửa draft/approved | Tên dễ nhận biết |
...
| timezone | TEXT | NOT NULL DEFAULT Asia/Ho_Chi_Minh | `z.string().min(1).max(64)` | select-enum [Asia/Ho_Chi_Minh:Giờ Việt Nam] `*` | `ianaTimezone`: "Múi giờ không hợp lệ" | Asia/Ho_Chi_Minh | all thấy; Manager+ sửa draft/approved | Khóa ngày công theo giờ server |
| shift1_start_minute | INTEGER | NOT NULL DEFAULT 420 CHECK 0..1439 | `z.number().int().min(0).max(1439)` | time `HH:mm` `*` ↔ phút lưu trữ | `minuteOfDay`: "Giờ bắt đầu ca 1 không hợp lệ" | 420 → 07:00 | all thấy; Manager+ sửa draft/approved | 07:00, đầu khoảng tính công ca 1 |
| shift1_end_minute | INTEGER | NOT NULL DEFAULT 690 CHECK > start | `z.number().int().min(1).max(1439)` | time `HH:mm` `*` ↔ phút lưu trữ | `orderedMinuteRange`: "Giờ kết thúc ca 1 phải sau giờ bắt đầu" | 690 → 11:30 | all thấy; Manager+ sửa draft/approved | 11:30, cuối khoảng tính công ca 1 |
| shift2_start_minute | INTEGER | NOT NULL DEFAULT 780 | `z.number().int().min(0).max(1439)` | time `HH:mm` `*` ↔ phút lưu trữ | `minuteOfDay`: "Giờ bắt đầu ca 2 không hợp lệ" | 780 → 13:00 | all thấy; Manager+ sửa draft/approved | 13:00 |
| shift2_end_minute | INTEGER | NOT NULL DEFAULT 1020 CHECK > start | `z.number().int().min(1).max(1439)` | time `HH:mm` `*` ↔ phút lưu trữ | `orderedMinuteRange`: "Giờ kết thúc ca 2 phải sau giờ bắt đầu" | 1020 → 17:00 | all thấy; Manager+ sửa draft/approved | 17:00 |
| shift3_start_minute | INTEGER | NOT NULL DEFAULT 1050 | `z.number().int().min(0).max(1439)` | time `HH:mm` `*` ↔ phút lưu trữ | `minuteOfDay`: "Giờ bắt đầu ca 3 không hợp lệ" | 1050 → 17:30 | all thấy; Manager+ sửa draft/approved | 17:30, toàn bộ là OT |
| shift3_latest_out_minute | INTEGER | NOT NULL DEFAULT 1439 CHECK > start | `z.number().int().min(1).max(1439)` | time `HH:mm` `*` ↔ phút lưu trữ | `orderedMinuteRange`: "Giờ ra tối đa ca 3 phải sau giờ vào" | 1439 → 23:59 | all thấy; Manager+ sửa draft/approved | Không tự tính qua ngày |
| regular_daily_cap_minutes | INTEGER | NOT NULL DEFAULT 480 CHECK 1..1440 | `z.number().int().min(1).max(1440)` | duration giờ + phút `*` ↔ phút lưu trữ | `regularCap`: "Phút công thường phải từ 1 đến 1440" | 480 → 8 giờ 0 phút | all thấy; Manager+ sửa draft/approved | Quá 480 phút ca 1+2 chuyển OT |
| qr_ttl_seconds | INTEGER | NOT NULL DEFAULT 15 CHECK 10..60 | `z.number().int().min(10).max(60)` | number `*` | `qrTtl`: "Chu kỳ QR phải từ 10 đến 60 giây" | 15 | all thấy; Manager+ sửa draft/approved | Chu kỳ phát QR |
| effective_from | TEXT | NOT NULL ISO date | `z.string().date()` | date `*` | `notPastWhenNew`: "Ngày hiệu lực không hợp lệ" | ngày bắt đầu do Owner chọn | all thấy; Manager+ sửa draft/approved | Chọn policy theo ngày công |
| effective_to | TEXT | NULL ISO date | `z.string().date().nullable()` | date | `effectiveRange`: "Ngày kết thúc phải sau ngày bắt đầu" | — | all thấy; Manager+ sửa draft/approved | Kết thúc version |
...
State machine: `draft → approved → retired`. Nút: draft `Lưu`, `Duyệt`; approved cho phép `Lưu` các trường cấu hình được khai báo `allow_on_submit` và `Ngừng áp dụng`; retired khóa sửa. Manager+ có thể `Xoá` chính sách ở mọi trạng thái sau bước xác nhận, nhưng server chỉ thực hiện khi không còn chứng từ tham chiếu và không có phát sinh sổ cái. Policy Slice 1 không có `approved_by`/`approved_at`; actor, thời gian và before/after của duyệt được lấy từ workflow/audit timeline. Slice 1 chưa chặn policy approved chồng khoảng hiệu lực; đây là validator bắt buộc của pha sau, còn scan hiện chỉ dùng policy mà trạm tham chiếu, ở trạng thái approved và còn hiệu lực.
...
# Cập nhật field ledger ngày 2026-08-11

Các dòng `qr_ttl_seconds`, challenge heartbeat và nhận diện thiết bị kiểu fingerprint trong ledger gốc đã được **thay thế** bởi ledger QR cố định/GPS/device credential tại [ALUMDOOR-ATTENDANCE-STATIC-QR-GPS-DEVICE-20260811.md](./ALUMDOOR-ATTENDANCE-STATIC-QR-GPS-DEVICE-20260811.md).
